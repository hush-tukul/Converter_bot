...
from sqlalchemy import create_engine, exists, BigInteger, delete, LargeBinary, DateTime, text
...
with engine.connect() as conn:
    conn.execute(text("create schema if not exists users_files;"))
...
def save_file(id, filetype, option, user_id, user_name):
    time = datetime.now()
    file_data = File(id=id, filetype=filetype, option=option, user_id=user_id, user_name=user_name, datetime=time)
    session.add(file_data)
    session.commit()


#save_file('dadqdqdqd1d1d', 'str', 'pdf', 121212)
